...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        // Performance is not good enough here (although passes locally)
        String retVal = Stream.of(studentArray)
            .parallel()
            .filter(s -> !s.checkIsCurrent()) // could also use predicate here
        ...
        // Neither is:
        // Map<String, Long> mapCounts = Stream.of(studentArray)
        //     .parallel()
        //     .filter(student -> !student.checkIsCurrent())
        //     .collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()))

        // String retVal = Collections.max(map.entrySet(), Map.Entry.comparingByValue())
        //     .getKey();
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        // Use predicates because some evidence that one complex filter performs
        // better than multiple simple filters
        Predicate<Student> isStudentCurrent = s -> s.checkIsCurrent();
        Predicate<Student> isFailingGrade = s -> s.getGrade() < 65;
        Predicate<Student> isOlderThan20 = s -> s.getAge() > 20;

        // one complex filter has slightly better performance than many simple filters
        long retVal = Stream.of(studentArray)
            .parallel()
            .filter(isStudentCurrent.negate().and(isFailingGrade).and(isOlderThan20))
            .count();
